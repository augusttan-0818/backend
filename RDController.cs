...
namespace NRC.Const.CodesAPI.API.Controllers
{
    ...
    public class ReferenceDocumentUpdateController : ControllerBase
    {
        ...
        [Produces("application/json")]
        public async Task<ActionResult<IEnumerable<ReferenceDocumentUpdateListDto>>> GetAllStandardUpdatesList()
        {
            var result = await _service.GetAllStandardUpdatesListAsync();
            return Ok(result);
        }
